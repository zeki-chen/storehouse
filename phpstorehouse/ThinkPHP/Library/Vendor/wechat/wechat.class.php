...
class Wechat {
	...
	public function __construct($options) {
		...
		$this->logfile = isset ( $options ['logfile'] ) ? $options ['logfile'] : dirname ( __FILE__ ) . "/../../../../log/wxlog" . date ( "Ymd" ) . ".txt";
	...
	public function log($log) {		
		...
		if ($this->debug) {
			if (function_exists ( $this->logcallback )) {				
				...
				$res=file_put_contents ( $this->logfile, date('Y-m-d H:i:s').":".$log . "\r\n", FILE_APPEND );				
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	public function getJsSign($url, $timestamp = 0, $noncestr = '', $appid = '') {
		...
		$this->log ( "arrdata:" . print_r ( $arrdata, true ) );
		...
		$this->log ( "signPackage:" . print_r ( $signPackage, true ) );
		return $signPackage;
	}
...
// 				return false;
	 ...
	 *
	 * @param array $data
	 *        	{"media":'@Path\filename.jpg'} 格式限定为：jpg,jpeg,png,gif，图片大小建议120px*120 px，限制不超过200 px *200 px，图片需为正方形。
	 * @return boolean|array {
	 *         "data": {
	 ...
	 *        	在摇一摇页面展示的图片， 格式限定为：jpg,jpeg,png,gif; 建议120*120 ， 限制不超过200*200
