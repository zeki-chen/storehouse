...
<script type="text/javascript" src="/xfzz/web/Public/Validform_v5.3.2/js/Validform_v5.3.2_min.js"></script>
		<link rel="stylesheet" type="text/css" href="/xfzz/web/Public/Index/css/details.css" />
	</head>
            ...
            <?php if($k < $breadLength-1): ?>&gt;&gt;<?php endif; endforeach; endif; ?>
					...
					<div class="essay">
						...
						<div class="info clearFloat">
							<?php if(!empty($art["reference"])): ?><span class="item">来源：<?php echo ($art["reference"]); ?></span><?php endif; ?>
							<span class="item">阅读量：<?php echo ($art["hits"]); ?></span>
							<span class="item">发布时间：<?php echo (date("Y-m-d",$art["pubdate"])); ?></span>
						</div>
						<?php if(!empty($art['video'])): ?><div style="width:520px;border: 1px solid #ccc;background-color: #f0f0f0;padding: 4px;margin:25px auto;">
                                <?php if($art['video_type'] == 1): ?><!-- 本地视频 -->
                                    <script type="text/javascript" src="/xfzz/web/Public/flowplayer/flowplayer-3.2.13.min.js"></script>
                                    <a href="<?php echo ($art['video'][0]['path']); ?>" style="display:block;width:520px;height:330px" id="player">
                                    	<div>
                                    		<img src="/xfzz/web/Public/flowplayer/play_large.png" style="margin: 124px 0 0 219px!important;position: absolute;">
                                    	</div>
                                    </a>
                                    <script>
						...
						<?php if(!empty($art['media'])): ?><div class="attachment">
								<div class="attachment_name">相关附件：</div>
								<?php if(is_array($art['media'])): foreach($art['media'] as $key=>$item): ?><div class="attachment_content">
										<a href="<?php echo U('Details/download',array('url'=>base64_encode($item['path']),'fileName'=>base64_encode($item['name'])));?>" class="download" title="<?php echo ($item["fileName"]); ?>"><?php echo ($item["name"]); ?></a>
